...
1. **Pending-confirmation check** ([[chat]] `PendingConfirmationService`). If the user has an in-flight destructive action, parse `yes`/`no` intent first so confirmation replies aren't scope-blocked.
...
4. **Build message array** from full session history. Critical: when assistant tool-call messages have no following tool responses (corrupted history), inject synthetic `{"status":"pending_confirmation"}` tool messages so DeepSeek won't 400.
...
`ToolExecutor.execute` strips `id` from args before forwarding to update calls (so it doesn't leak into Prisma's data payload). It catches all errors and returns `{ error: message }` instead of throwing — the LLM sees the error and recovers.
...
- PHP (₱) for all amounts.
- Never directly execute the destructive tools — describe and request confirmation (the system enforces this regardless).
