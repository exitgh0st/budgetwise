...
## Purpose
AI financial advisor. Wraps DeepSeek V3 (via the `openai` SDK), exposes 25 tools, runs prompt-injection + scope + output guardrails, and gates destructive actions behind explicit user confirmation.
...
| `tools/tool-definitions.ts` | OpenAI-compatible JSON schemas for all 25 tools |
