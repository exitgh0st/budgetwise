...
## Tech Stack
- **Frontend:** Angular 20 (standalone components, signals), Angular Material (M3 violet/rose), Chart.js via `ng2-charts`, RxJS
- **Backend:** NestJS 11, Prisma ORM, PostgreSQL, Passport JWT (ES256 via Supabase JWKS), `@nestjs/schedule` (hourly cron)
- **AI:** DeepSeek V3 via the `openai` SDK (`baseURL=https://api.deepseek.com`), 25 tool definitions, custom guardrails + destructive-action confirmation
